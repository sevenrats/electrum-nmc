...
<ConfirmTxDialog@Popup>
    ...
    BoxLayout:
        ...
        BoxLayout:
            orientation: 'horizontal'
            size_hint: 1, (0.5 if root.name_fee else 0.01)
            Label:
                text: _('Name registration fee') if root.name_fee else ''
            Label:
                text: root.name_fee
        BoxLayout:
            orientation: 'horizontal'
...
class ConfirmTxDialog(FeeSliderDialog, Factory.Popup):
    ...
    def update_tx(self):
        ...
        tx.set_rbf(rbf)
        amount = sum(map(lambda x: x.value_display, outputs)) if '!' not in [x.value_display for x in outputs] else tx.output_value_display()
        fee = tx.get_fee()
        fee_display = tx.get_fee_display()
        name_fee = None if (fee is None or fee_display is None) else (fee_display - fee)
        if name_fee:
            self.name_fee = self.app.format_amount_and_units(name_fee)
        else:
            self.name_fee = ''
        feerate = Decimal(fee) / tx.estimated_size()  # sat/byte
        self.ids.fee_label.text = self.app.format_amount_and_units(fee) + f' ({feerate:.1f} swartz/B)'
        ...
        if fee_ratio >= FEE_RATIO_HIGH_WARNING:
            ...
            self.warning = _('Warning') + ': ' + _("The fee for this transaction seems unusually high.") + f' (feerate: {feerate:.2f} swartz/byte)'
        else:
            self.warning = ''
        self.tx = tx
