...
class TestLightning(unittest.TestCase):
    ...
    def run_shell(args, timeout=30):
        process = subprocess.Popen(['electrum_nmc/electrum/tests/regtest/regtest.sh'] + args, stderr=subprocess.STDOUT, stdout=subprocess.PIPE, universal_newlines=True)
...
class TestNamecoinABDefaultSeed(TestLightning):
    ...
    def test_name_ant_workflow(self):
        self.run_shell(['name_ant_workflow'])
